// DeepPotHelper.cpp
#include "DeepPotHelper.h"
#include "tensorflow/core/platform/env.h"
#include "device.h" // for DPGetDeviceCount, DPSetDevice
#include <algorithm>
#include <stdexcept>
#include "common.h" 
...
template <typename ENERGYTYPE, typename VALUETYPE>
static void run_model(
  ...
  run_model<ENERGYTYPE, VALUETYPE>(dener_, dforce_, dvirial, datom_energy_,
...
DeepPotHelper::DeepPotHelper() 
    : inited(false), init_nbor(false), graph_def(new GraphDef()) {}


DeepPotHelper::DeepPotHelper(const std::string& model, int gpu_rank, const std::string& file_content)
    : inited(false), init_nbor(false), graph_def(new GraphDef()) {
  try {
...
// 模拟 DeepPotTF::get_scalar
template <class VT>
VT DeepPotHelper::get_scalar(const std::string& name) const {
    return deepmd::session_get_scalar<VT>(session, name);
}

void DeepPotHelper::init(const std::string& model, int gpu_rank, const std::string& file_content) {
    if (inited) {
    ...
    deepmd::DPGetDeviceCount(gpu_num);
    if (gpu_num > 0) {
        options.config.set_allow_soft_placement(true);
        options.config.mutable_gpu_options()->set_per_process_gpu_memory_fraction(0.9);
        options.config.mutable_gpu_options()->set_allow_growth(true);
        deepmd::DPSetDevice(gpu_rank % gpu_num);
        // 设置 Graph 的默认设备
        std::string device_str = "/gpu:" + std::to_string(gpu_rank % gpu_num);
        graph::SetDefaultDevice(device_str, graph_def);
    }
#endif
  deepmd::check_status(NewSession(options, &session));
  deepmd::check_status(session->Create(*graph_def));
  try {
    model_version = get_scalar<std::string>("model_attr/model_version");
  } catch (deepmd::tf_exception& e) {
    // no model version defined in old models
    model_version = "0.0";
  }
  std::string global_model_version = "1.0";
  ...
  model_type = get_scalar<std::string>("model_attr/model_type");
  inited = true;
...
void DeepPotHelper::validate_fparam_aparam(const int& nframes,
                                           const int& nloc,
                                           const std::vector<VALUETYPE>& fparam,
                                           const std::vector<VALUETYPE>& aparam) const {
    if (fparam.size() != static_cast<size_t>(dfparam) && fparam.size() != static_cast<size_t>(nframes) * dfparam) {
        throw std::runtime_error("the dim of frame parameter provided is not consistent with what the model uses");
    }
    if (aparam.size() != static_cast<size_t>(daparam_) * nloc &&
        aparam.size() != static_cast<size_t>(nframes) * daparam_ * nloc) {
        throw std::runtime_error("the dim of atom parameter provided is not consistent with what the model uses");
    }
}
template void DeepPotHelper::validate_fparam_aparam<double>(
    const int& nframes,
    const int& nloc,
    const std::vector<double>& fparam,
    const std::vector<double>& aparam) const;
template void DeepPotHelper::validate_fparam_aparam<float>(
    const int& nframes,
    const int& nloc,
...
template <typename VALUETYPE>
void DeepPotHelper::tile_fparam_aparam(std::vector<VALUETYPE>& out_param,
                                       const int& nframes,
                                       const int& dparam,
    ...
    if (param.size() == dparam) {
        ...
        for (int ii = 0; ii < nframes; ++ii) {
            std::copy(param.begin(), param.end(), out_param.begin() + static_cast<unsigned long>(ii) * dparam);
        }
    } else if (param.size() == static_cast<size_t>(nframes) * dparam) {
        out_param = param;
    }
}
template void DeepPotHelper::tile_fparam_aparam<double>(
    std::vector<double>& out_param,
    const int& nframes,
    const int& dparam,
    const std::vector<double>& param) const;

template void DeepPotHelper::tile_fparam_aparam<float>(
    std::vector<float>& out_param,
    const int& nframes,
    const int& dparam,
    const std::vector<float>& param) const;

void DeepPotHelper::compute(double& dener,
                            ...
                            const std::vector<double>& aparam__) 
{
    std::vector<double> dener_;
    std::vector<double> datom_energy_, datom_virial_;
    
    int nall = datype_.size();
    int nframes = nall > 0 ? (dcoord_.size() / nall / 3) : 1;
    int nloc = nall - nghost;

    std::vector<double> fparam, aparam;
    ...
    if (ago == 0) {
        atommap_ = deepmd::AtomMap(datype_.begin(), datype_.begin() + nloc_real);
